...
ORDER_FORM_DATE = (TODAY - timedelta(days=182)).strftime("%Y-%m-%d")  # 6 months + 2 days ago
MEMO_DATE = (TODAY - timedelta(days=120)).strftime("%Y-%m-%d")  # 4 months ago
...
def run_basic_test():
    ...
    with cleanup_context(cleanup_list):
        try:
            ...
            test_supplier_input = {"name": TEST_SUPPLIER_NAME,
                                   "address": "test_address",
                                   "phone_number": "9350544808"}

            test_party_input = {"name": TEST_PARTY_NAME,
                                "address": "test_address",
            ...
            bill_input = {"bill_number": TEST_BILL_NUMBER,
                          "amount": bill_amount,
                          "supplier_id": test_supplier_id,
                          "party_id": test_party_id,
            ...
            register_entry = RegisterEntry.retrieve(
                test_supplier_id, test_party_id, TEST_BILL_NUMBER, BILL_DATE)
            ...
            report = TestKhataReport("khata_report", [test_party_id], [
                test_supplier_id], BILL_DATE, BILL_DATE)
            ...
            reference = {'title': 'Payment List', 'from': BILL_DATE, 'to': BILL_DATE, 
                        'headings': [{'title': f'Party Name: {TEST_PARTY_NAME}', 
                                    'subheadings': [{'title': f'Supplier Name: {TEST_SUPPLIER_NAME}', 
                                                   'dataRows': [{'part_no': '', 'part_date': '', 'part_amt': '', 
                                                               'bill_no': int(TEST_BILL_NUMBER), 'bill_amt': '5,000', 
                                                               'bill_date': format_date_for_display(BILL_DATE), 
                                                               'pending_amt': 5000, 'days': DAYS_SINCE_BILL, 'status': 'N'}], 
                                                   'specialRows': [{'name': 'Total (=)', 'value': '0', 'column': 'part_amt', 'numeric': 0, 'beforeData': False}, 
                                                                 ...
                                                                 {'name': '>120 days (+)', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}, 
                                                                 {'name': 'Subtotal (=)', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}, 
                                                                 {'name': 'Part (-)', 'value': '- 0', 'column': 'bill_amt', 'numeric': 0, 'beforeData': False}, 
                                                                 {'name': 'Pending (=)', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}], 
                                                   'displayOnIndex': True, 
                                                   'cumulative': {'name': 'Total Pending', 'value': '5,000'}}], 
                                    'cumulative': {'name': 'Total Pending', 'value': '5,000'}}]}
            ...
            # Create a Memo Entry
            part_input = {'memo_number': TEST_MEMO_NUMBER1,
            ...
            part_memo = MemoEntry.retrieve(
                test_supplier_id, test_party_id, TEST_MEMO_NUMBER1)
            ...
            reference = {'title': 'Payment List', 'from': BILL_DATE, 'to': BILL_DATE, 
                        'headings': [{'title': f'Party Name: {TEST_PARTY_NAME}', 
                                    'subheadings': [{'title': f'Supplier Name: {TEST_SUPPLIER_NAME}', 
                                                   'dataRows': [{'part_no': TEST_MEMO_NUMBER1, 
                                                               'part_date': format_date_for_display(MEMO_DATE), 
                                                               'part_amt': '1,250', 
                                                               'bill_no': int(TEST_BILL_NUMBER), 
                                                               'bill_amt': '5,000', 
                                                               'bill_date': format_date_for_display(BILL_DATE), 
                                                               'pending_amt': 5000, 
                                                               'days': DAYS_SINCE_BILL, 
                                                               'status': 'N'}], 
                                                   'specialRows': [{'name': 'Total (=)', 'value': '1,250', 'column': 'part_amt', 'numeric': 1250, 'beforeData': False}, 
                                                                 {'name': '<60 days (+)', 'value': '0', 'column': 'bill_amt', 'numeric': 0, 'beforeData': False}, 
                                                                 {'name': '60-120 days (+)', 'value': '0', 'column': 'bill_amt', 'numeric': 0, 'beforeData': False}, 
                                                                 {'name': '>120 days (+)', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}, 
                                                                 {'name': 'Subtotal (=)', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}, 
                                                                 {'name': 'Part (-)', 'value': '- 1,250', 'column': 'bill_amt', 'numeric': 1250, 'beforeData': False}, 
                                                                 {'name': 'Pending (=)', 'value': '3,750', 'column': 'bill_amt', 'numeric': 3750, 'beforeData': False}], 
                                                   'displayOnIndex': True, 
                                                   'cumulative': {'name': 'Total Pending', 'value': '3,750'}}], 
                                    'cumulative': {'name': 'Total Pending', 'value': '3,750'}}]}
            ...
            full_input = {'memo_number': TEST_MEMO_NUMBER2,
                          'register_date': MEMO_DATE,
                          'amount': amount,
                          'gr_amount': gr_amount,
                          'deduction': deduction,
            ...
            full_memo = MemoEntry.retrieve(
                test_supplier_id, test_party_id, TEST_MEMO_NUMBER2)
            ...
            reference = {'title': 'Supplier Register', 'from': BILL_DATE, 'to': BILL_DATE, 
                        'headings': [{'title': f'Supplier Name: {TEST_SUPPLIER_NAME}', 
                                    'subheadings': [{'title': '', 
                                                   'dataRows': [{'bill_date': format_date_for_display(BILL_DATE), 
                                                               'party_name': TEST_PARTY_NAME, 
                                                               'bill_no': int(TEST_BILL_NUMBER), 
                                                               'bill_amt': '5,000', 
                                                               'pending_amt': '0', 
                                                               'status': 'F'}], 
                                                   'specialRows': [{'name': 'Total (=) ', 'value': '5,000', 'column': 'bill_amt', 'numeric': 5000, 'beforeData': False}, 
            ...
            try:
                ...
                print_dict_diff(
                    og_payment_report['headings'], {"headings": []})
                raise
