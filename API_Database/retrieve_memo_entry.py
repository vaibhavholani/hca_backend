...
def check_new_memo(memo_number: int, 
    ...
    if len(result) > 1:
        raise DataError(f"Error with memo_number {memo_number}, more than one entries in memo_entry.")
    if len(result) == 0:
        return True
...
def get_id_by_memo_number(memo_number: int, supplier_id: int, party_id: int) -> int:
    """
