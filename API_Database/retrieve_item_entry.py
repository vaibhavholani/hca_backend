...
def get_all_item_entries(register_entry_id: int=None, item_id: int=None) -> List[Dict]:
    ...
    if register_entry_id:
        select_query = select_query.where(item_entry_table.register_entry_id == register_entry_id)
    if item_id:
...
def retrieve_item_entry(register_entry_id: int, item_id: int) -> Union[List[Dict], Dict]:
    """Retrieves an item entry based on register entry ID and item ID; raises DataError if multiple entries are found."""
    result = get_all_item_entries(register_entry_id, item_id)
    if register_entry_id and item_id:
        if len(result) != 1:
            raise DataError(f'Multiple Item Entries found with given criteria: Register Entry id: {register_entry_id}, Item id: {item_id}')
        else:
            return result[0]
    return result
...
def get_item_entry_id(register_entry_id: int, item_id: int) -> int:
    ...
    if len(result) == 0:
        raise DataError(f'No Item Entry found with given criteria: Register Entry id: {register_entry_id}, Item id: {item_id}')
