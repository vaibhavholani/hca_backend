...
class TestKhataTable(KhataReport):
    ...
    def __init__(self) -> None:
        """Initializes the test report with default register columns."""
        super().__init__()
        self.register_columns = ['register_date', 'bill_number', 'amount', 'status']

    def generate_data_rows(self, header_id: int, subheader_id: int, start_date: str, end_date: str, register_entries: List[RegisterEntry], memo_entries: List[MemoEntry]) -> Tuple[List[Dict], List[Dict]]:
        """
            ...
            for memo_entry in memo_entries:
                for bill in memo_entry.memo_bills:
                    if bill.bill_id == register_entry.get_id():
                        add_dummy = False
                        memo_dict = {'memo_no': memo_entry.memo_number, 'memo_amt': bill.amount, 'memo_date': self._format_date(memo_entry.register_date), 'chk_amt': memo_entry.amount, 'memo_type': bill.type}
                        if first:
...
class TestKhataReport(Report):
    ...
    def __init__(self, title: str, party_ids: List[int], supplier_ids: List[int], start_date: str, end_date: str) -> None:
        """Initializes the test report with default register columns."""
        super().__init__(title=title, party_ids=party_ids, supplier_ids=supplier_ids, start_date=start_date, end_date=end_date, test_mode=True)
        self.table = TestKhataTable()

    def generate_table(self, register_entries: List[RegisterEntry], memo_entries: List[MemoEntry]):
        """Generates a report table from register and memo entries and returns the structured report data."""
        all_data = {}
        ...
        for header_id in self.header_ids:
            ...
            for subheader_id in filter_subheaders:
                (data_rows, special_rows, cumulative) = self.table.generate_data_rows(header_id, subheader_id, self.start_date, self.end_date, register_entries, memo_entries)
                if len(data_rows) != 0:
